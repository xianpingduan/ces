...
public class PullListView extends ListView implements OnScrollListener
{
    ...
    private final String Tag = "pada.widget.PadaPullListView";
    ...
    {
	this( context , attrs , R.style.NormalList );
    }
	...
	if( mIsHeaderAdded == false && enable )
	{
	    mIsHeaderAdded = true;
	    // init header view
	    mHeaderView = new ListViewHeader( getContext( ) , true );
	    mHeaderViewContent = (RelativeLayout)mHeaderView.findViewById( R.id.pl_listview_header_content );
	    mHeaderTimeView = (TextView)mHeaderView.findViewById( R.id.pl_listview_header_time );
	...
	if( mHeaderPullEnable && !mHeaderPullRefreshing )
	{ // 未处于刷新状态，更新箭头
	...
	//		Log.d(Tag, "updateFooterHeight height=" + mFooterView.getVisiableHeight() + "|mFooterViewHeight=" + mFooterViewHeight + "|" + mFooterPullRefreshing
	//				+ "|" + delta);

	if( mFooterPullEnable && !mFooterPullRefreshing )
	{ // 未处于刷新状态，更新箭头
	...
	//Log.d(Tag, "onTouchEvent getAction = " + ev.getAction() + "|MotionEvent=" + ev.toString());
	...
	switch ( ev.getAction( ) )
	{
		...
		mLastY = ev.getRawY( );
		if( getFirstVisiblePosition( ) == 0 && mHeaderPullEnable && ( mHeaderView != null ) && ( mHeaderView.getVisiableHeight( ) > 0 || deltaY > 0 ) )
		{
		    // the first item is showing, header has shown or pull down.
		    // Log.d(Tag, "MotionEvent.ACTION_MOVE updateHeaderHeight");
		    updateHeaderHeight( deltaY / OFFSET_RADIO );
		    invokeOnScrolling( );
		}
		else if( getLastVisiblePosition( ) == mTotalItemCount - 1 && mFooterPullEnable && ( mFooterView != null ) && ( mFooterView.getVisiableHeight( ) > 0 || deltaY < 0 ) )
		{
		    // last item, already pulled up or want to pull up.
		    //Log.d(Tag, "MotionEvent.ACTION_MOVE updateFooterHeight deltaY=" + deltaY);
	    ...
	    default :
		mLastY = -1; // reset
		if( getFirstVisiblePosition( ) == 0 && mHeaderPullEnable && mHeaderView != null )
		{
		    // invoke refresh
		    if( mHeaderPullEnable && ( mHeaderView.getVisiableHeight( ) > mHeaderViewHeight ) && ( mHeaderView.getState( ) != ListViewHeader.STATE_REFRESHING ) )
		    {
		...
		else if( getLastVisiblePosition( ) == mTotalItemCount - 1 && mFooterPullEnable && mFooterView != null )
		{
		    // invoke refresh
		    //Log.d(Tag, "mFooterViewHeight " + mFooterViewHeight + "|mFooterView.getVisiableHeight()=" + mFooterView.getVisiableHeight());
		    if( mFooterPullEnable && ( mFooterView.getVisiableHeight( ) > mFooterViewHeight ) && ( mFooterView.getState( ) != ListViewHeader.STATE_REFRESHING ) )
		    {
			mFooterPullRefreshing = true;
			mFooterView.setState( ListViewHeader.STATE_REFRESHING );
			if( mListViewListener != null )
			{
