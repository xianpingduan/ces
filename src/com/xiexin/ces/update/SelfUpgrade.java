...
public class SelfUpgrade
{
	...
	HttpHandler< File > mHandler = http.download( packUrl, filePath + getSaveFileName( mContext ) , true , false , new RequestCallBack< File >( )
	{
	    @Override
	    public void onSuccess( ResponseInfo< File > responseInfo )
	    {
		isDownloading = false;
		LogUtils.d( "文件" + responseInfo.result.getPath( ) + "下载完成，开始安装!" );
		finishDownloadNotification( responseInfo.result );
		//安装
		PackageUtils.installNormal( mContext , responseInfo.result.getPath( ) );
		//统计
	    }
		...
		//		if( mUpdateListener != null )
		//		    mUpdateListener.onLoading( total , current , isUploading );
	    }
	...
	StringBuffer urlSbf = new StringBuffer( App.getRootUrl( ) + Constants.UPDATE + "?" );
	...
	public void handleMessage( Message msg )
	{
		...
		case MSG_HAVE_UPGRADE_TO_INSTALL :
			...
			try {
				mUpdateInfo = new JSONObject(info);
			} catch (JSONException e) {
				e.printStackTrace();
			}
		...
		case MSG_NO_HAVE_UPGRADE_TO_INSTALL :
		    ...
		    if( mSelfUpdateListener != null )
		    {
			mSelfUpdateListener.checkUnUpdate( MSG_NO_HAVE_UPGRADE_TO_INSTALL );
		    }
		    break;
	...
	if( mUpgradeDialog == null )
	    mUpgradeDialog = new SelfUpdateCommonDialog( mContext , mUpdatePrompt==null?"":mUpdatePrompt , mUpdateDesc==null?"":mUpdateDesc );
	mUpgradeDialog.setmRightBtnTitle( mContext.getString( ResourceUtil.getStringId( mContext , "self_update_dialog_right_title" ) ) );
