...
public class LoginActivity extends Activity implements OnClickListener {
	...
	private void initData() {
		...
		if (remeberPwd) {
			...
			String userid = App.getSharedPreference().getString(
					Constants.USER_ID, "");
			String pwd = App.getSharedPreference().getString(Constants.PWD, "");
			String connName = App.getSharedPreference().getString(
					Constants.ZHANG_TAO_CONN_NAME, "");

			boolean rememberPwd = App.getSharedPreference().getBoolean(
					Constants.REMEBER_PWD, false);
						...
						try {
							...
							if (resCode == 0) {
								App.getSharedPreference()
										.edit()
										.putString(Constants.ZHANG_TAO_LIST,
												response.getString("Data"))
										.commit();
								mUiHandler
										.sendEmptyMessage(MSG_REQUEST_ZT_SUCCESS);
							} else {
								mUiHandler
										.sendEmptyMessage(MSG_REQUEST_ZT_ERROR);
							}
						} catch (JSONException e) {
							e.printStackTrace();
						}
	...
	private boolean validate() {
		...
		if (account == null || account.isEmpty()) {
			Toast.makeText(mContext, getString(R.string.please_enter_account),
					Toast.LENGTH_SHORT).show();
			return false;
		} else if (pwd == null || pwd.isEmpty()) {
			Toast.makeText(mContext, getString(R.string.please_enter_pwd),
					Toast.LENGTH_SHORT).show();
			return false;
		} else if (zt == null || zt.isEmpty()
				|| zt.equals(getString(R.string.p_select_zt_text))) {
			Toast.makeText(mContext, getString(R.string.please_select_zt),
					Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
		...
		public void handleMessage(Message msg) {
			...
			case MSG_REQUEST_ZT_ERROR:
				...
				Toast.makeText(LoginActivity.this,
						getString(R.string.get_accout_error),
						Toast.LENGTH_SHORT).show();
				break;
	...
	private void saveLoginInfo(String loginInfo) {
		...
		try {
			...
			App.getSharedPreference().edit()
					.putString(Constants.USER_ID, mUserId).commit();
			App.getSharedPreference()
					.edit()
					.putString(Constants.USER_NAME,
							userName == null ? "" : userName).commit();
			App.getSharedPreference().edit().putString(Constants.PWD, mPwd)
					.commit();
			App.getSharedPreference().edit()
					.putString(Constants.PWD_MD5, mPwdMd5).commit();
			App.getSharedPreference().edit()
					.putString(Constants.DEPART, depart).commit();
			App.getSharedPreference().edit().putString(Constants.TITLE, title)
					.commit();
			App.getSharedPreference().edit().putString(Constants.JOB, job)
					.commit();
			App.getSharedPreference().edit()
					.putBoolean(Constants.LOCKED, locked).commit();
			App.getSharedPreference()
					.edit()
					.putString(Constants.ZHANG_TAO_ACCINFO,
							mZtTv.getText().toString()).commit();
			App.getSharedPreference()
					.edit()
					.putString(Constants.ZHANG_TAO_CONN_NAME,
							mZtTv.getTag().toString()).commit();

			if (mRemmenberPwdCb.isChecked()) {
				App.getSharedPreference().edit()
						.putBoolean(Constants.REMEBER_PWD, true).commit();
			}
			if (mAutoLoginCb.isChecked()) {
				App.getSharedPreference().edit()
						.putBoolean(Constants.AUTO_LOGIN, true).commit();
			}
	...
	private void showPwd(View v) {
		...
		if (hidePwd) {
			mLoginPwdEt.setTransformationMethod(HideReturnsTransformationMethod
					.getInstance());
			mShowCb.setChecked(true);
		} else {
			mLoginPwdEt.setTransformationMethod(PasswordTransformationMethod
					.getInstance());
