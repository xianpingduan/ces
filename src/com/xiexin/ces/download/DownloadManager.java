package com.xiexin.ces.download;
...
import com.xiexin.ces.utils.SDCardUtils;
...
public final class DownloadManager {
	...
	static {
		...
		ColumnConverterFactory.registerColumnConverter(DownloadState.class,
				new DownloadStateConverter());
	}

	private static DownloadManager instance;

	private final static int MAX_DOWNLOAD_THREAD = 2; // 有效的值范围[1, 3], 设置为3时,
														// 可能阻塞图片加载.
	...
	private final ConcurrentHashMap<DownloadInfo, DownloadCallback> callbackMap = new ConcurrentHashMap<DownloadInfo, DownloadCallback>(
			5);

	private DownloadManager() {
		DbManager.DaoConfig daoConfig = new DbManager.DaoConfig().setDbName(
				"download").setDbVersion(1);
		db = x.getDb(daoConfig);
		try {
			List<DownloadInfo> infoList = db.selector(DownloadInfo.class)
					.findAll();
			if (infoList != null) {
				for (DownloadInfo info : infoList) {
					if (info.getState().value() < DownloadState.FINISHED
							.value()) {
	...
	public DownloadInfo getDownloadInfo(Context context, AttachMent attachMent) {
		for (DownloadInfo di : downloadInfoList) {
			if (attachMent.getFilepath().equals(di.getUrl())) {
				return di;
			}
		}
		String fileName = attachMent.getAttchname();
		DownloadInfo downloadInfo = new DownloadInfo();
		downloadInfo.setUrl(changeToUrl(attachMent.getFilepath()));
		downloadInfo.setAutoRename(false);
		downloadInfo.setAutoResume(true);
		downloadInfo.setLabel(fileName);
		downloadInfo.setFileSavePath(SDCardUtils.getDownloadDiskCacheFilePath(
				context, fileName));
		return downloadInfo;
	}
	...
	public synchronized void startDownload(String url, String label,
		...
		if (downloadInfo != null) {
			...
			if (callback != null) {
				if (viewHolder == null) {
					viewHolder = new DefaultDownloadViewHolder(null,
							downloadInfo);
				}
		...
		if (downloadInfo == null) {
			...
			downloadInfo.setUrl(changeToUrl(url));
